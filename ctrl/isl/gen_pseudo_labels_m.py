...
from ctrl.isl.get_data_loader_for_isl import get_mapillary_train_dataloader


def gen_sudo_labels(model, cfg):
    model.eval()
    print()
    print('ctrl/isl/gen_pseudo_labels_m.py --> gen_sudo_labels(...)')
    print()
    ...
    save_root = os.path.join(cfg.TRAIN.PSEUDO_LABELS_DIR, 'mapillary', 'train') # arxiv version
    ...
    for i_iter in range(0, target_train_dataset_num_examples+1):
        with torch.no_grad():
            ...
            save_path_nparray = save_path
            save_path_images = save_path_imgs
            ...
            str1 = img_name_target[0].split('.')
            str2 = '{}.npy'.format(str1[0])
            ...
            filename2 = os.path.join(save_path_images, '{}.png'.format(str1[0]))
