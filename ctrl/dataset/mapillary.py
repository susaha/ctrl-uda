...
class MapillaryDataSet(BaseDataset):
    def __init__(self, root, list_path, set='train',
        ...
        print('ctrl/dataset/mapillary.py --> class MapillaryDataSet')
        self.info = json_load(info_path)
        self.map_vector = None
        self.scale_label = scale_label
        ori_class_names = [label['readable'] for label in self.info['labels']]
        if class_mappings is not None:
            self.map_vector = array_from_class_mappings(ori_class_names, class_mappings, class_list)
    ...
    def get_metadata(self, name, mode=None):
        ...
        if self.cfg.IS_ISL_TRAINING and self.set == 'train':
            str1 = name.split('.')
            # arxiv version
            label_file = os.path.join(self.cfg.TRAIN.PSEUDO_LABELS_DIR, 'mapillary', 'train', self.cfg.PSEUDO_LABELS_SUBDIR,
                                      'nparrays_{:.1f}'.format(self.cfg.ISL_THRESHOLD), '{}.npy'.format(str1[0]))
            # cvpr submitted version
            # label_file = os.path.join(self.root, self.set, 'sudo_label', self.cfg.MODEL_FOR_SUDO_LABEL_TRAINING,
            #                           'nparrays_{:.1f}'.format(self.cfg.ISL_THRESHOLD), '{}.npy'.format(str1[0]))
        else:
    ...
    def __getitem__(self, index):
        ...
        if self.cfg.IS_ISL_TRAINING and self.set == 'train':
            with open(label_file, 'rb') as f:
                label = np.load(f)
                if self.cfg.DEBUG:
                    label = cv2.resize(label, tuple(self.labels_size), interpolation=cv2.INTER_NEAREST)
                label = label.copy()
        else:
            label = Image.open(label_file)
            if self.scale_label:
                label = resize_with_pad(self.image_size, label, Image.NEAREST, fill_value=self.bg_idx)
            else:
                label = pad_with_fixed_AS(self.image_size[0] / self.image_size[1], label, fill_value=self.bg_idx)
